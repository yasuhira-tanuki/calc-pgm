...
## AI エージェント連携 (agent-tools)

calc-pgm を AI エージェントから利用するための **Rust 製の単一バイナリ MCP サーバー**を `agent-tools/` に置く。
計算エンジン(C)をバイナリに静的に埋め込み、FFI で直接呼び出す(プロセス生成なし)。本体の計算ロジックは
不変に保ち、埋め込みと表示整形のための薄い C シムのみを追加する。設計・使い方は [agent-tools.md](agent-tools.md) を参照。
